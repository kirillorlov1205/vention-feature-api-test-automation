...
import {
    createCustomerGarageVehicle,
    deleteCustomerGarageVehicleByVin,
    getCustomerGarageVehicles,
    updateCustomerGarageVehicleByVin
} from "../../src/methods/customerGarage";
import {
...
test.describe("Customer garage. Create customer garage vehicle", () => {
    ...
    test("I can create a new vehicle in customer garage", async ({request}) => {
        const response = await createCustomerGarageVehicle({request}, authToken);
        const responseBody = JSON.parse(await response.text());
        await deleteCustomerGarageVehicleByVin({request}, authToken, VALID_VEHICLE.vin);
...
test.describe("Customer garage. Get customer garage vehicles", () => {
    ...
    test("I can get customer garage vehicles", async ({request}) => {
        await createCustomerGarageVehicle({request}, authToken);
        const response = await getCustomerGarageVehicles({request}, authToken);
        const responseBody = JSON.parse(await response.text());
        await deleteCustomerGarageVehicleByVin({request}, authToken, VALID_VEHICLE.vin);
...
test.describe("Customer garage. Update customer garage vehicle", () => {
    ...
    test("I can update customer garage vehicle", async ({request}) => {
        await createCustomerGarageVehicle({request}, authToken);
        const response = await updateCustomerGarageVehicleByVin({request}, authToken, VALID_VEHICLE.vin, VEHICLES_LIST.fordFusion)
        const responseBody = JSON.parse(await response.text());
        await deleteCustomerGarageVehicleByVin({request}, authToken, VALID_VEHICLE.vin);
...
test.describe("Customer garage. Delete customer garage vehicle", () => {
    ...
    test("I can delete customer garage vehicle", async ({request}) => {
        await createCustomerGarageVehicle({request}, authToken);
        const response = await deleteCustomerGarageVehicleByVin({request}, authToken, VALID_VEHICLE.vin);
        const responseBody = JSON.parse(await response.text());
        expect(response.status()).toBe(200);
    ...
    test("I cannot delete customer garage vehicle by not existing vin", async ({request}) => {
        await createCustomerGarageVehicle({request}, authToken);
        const response = await deleteCustomerGarageVehicleByVin({request}, authToken, "notExistingVin");
        const responseBody = JSON.parse(await response.text());
        expect(response.status()).toBe(200);
    ...
    test("I cannot delete customer garage vehicle by vin less than limit characters", async ({request}) => {
        await createCustomerGarageVehicle({request}, authToken);
        const response = await deleteCustomerGarageVehicleByVin({request}, authToken, INVALID_VEHICLE_VIN_LIST["Less than limit characters"]);
        const responseBody = JSON.parse(await response.text());
    ...
    test("I cannot delete customer garage vehicle by vin more than limit characters", async ({request}) => {
        await createCustomerGarageVehicle({request}, authToken);
        const response = await deleteCustomerGarageVehicleByVin({request}, authToken, INVALID_VEHICLE_VIN_LIST["More than limit characters"]);
        const responseBody = JSON.parse(await response.text());
        expect(response.status()).toBe(400);
    ...
    test("I cannot delete customer garage vehicle by emty vin", async ({request}) => {
        await createCustomerGarageVehicle({request}, authToken);
        const response = await deleteCustomerGarageVehicleByVin({request}, authToken, "");
